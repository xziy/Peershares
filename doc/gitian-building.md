...
*Setup instructions for a gitian build of Franko using a Debian VM or physical system.*
...
- [Building Franko](#building-franko)
...
Clone the git repositories for franko and gitian and then checkout the franko version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the franko repository
...
To build Franko (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the franko repository.
...
For example:
```bash
URL=https://github.com/user/franko.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/franko-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/franko-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/franko-build.assert
```
...
[franko/gitian.sigs](https://github.com/franko-org/gitian.sigs.frk/) repository, or if that's not possible create a pull
request. 
